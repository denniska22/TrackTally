...
Die Wiedergabe startet für jeden Versuch mit einem einzelnen Befehl an derselben Songposition. Der Ausschnitt-Timer beginnt bei bestätigter Wiedergabe, unabhängig von der Antwort auf den Startbefehl. Eine zusätzliche Startfrist begrenzt die Wiedergabe auch bei ausbleibenden Player-Meldungen. Beim Ausschnittende, bei Fehlern und beim Tabwechsel pausiert das lokale Spotify SDK sofort und setzt die Lautstärke auf null. Vor dem nächsten Ausschnitt werden ausstehende Lautstärkebefehle und bereits angenommene Startbefehle innerhalb begrenzter Fristen abgeschlossen. Verzögerte SDK-Meldungen erhalten Zeit zur Bestätigung; bleibt diese aus, stoppt ein gerätespezifischer API-Befehl unabhängig von der Songsuche. Auch eine erfolgreiche API-Antwort bestätigt den Stopp. Nicht bestätigte Stopps oder hängende Befehle trennen die Player-Verbindung. Wiederholte Stopps eines bereits beendeten Ausschnitts senden keine weiteren Befehle. Abgebrochene Startbefehle werden nicht nachträglich aus der Warteschlange gesendet. Browser-, Netzwerk- und SDK-Latenz können besonders bei sehr kurzen Ausschnitten die tatsächliche Dauer beeinflussen; eine samplegenaue Grenze ist beim Spotify-Stream nicht garantiert. Technische Wiedergabefehler verbrauchen keinen Versuch. Der Spotify-Login kehrt über die bereits registrierte Root-Adresse zu diesem Modus zurück; eine zusätzliche Redirect URI ist nicht erforderlich.
